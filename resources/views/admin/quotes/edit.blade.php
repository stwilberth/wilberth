...
<body class="bg-slate-50 min-h-screen">
    ...
    <main class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <form method="POST" action="/admin/quotes/{{ $quote->id }}" id="quote-form" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 space-y-4">
                ...
                <div class="border-t border-slate-100 pt-4 space-y-2">
                    <div class="flex justify-between text-sm text-slate-600">
                        <span>Subtotal</span>
                        <span id="subtotal">₡{{ number_format($quote->subtotal, 0, ',', '.') }}</span>
                    </div>
                    <div class="flex justify-between text-sm text-slate-600">
                        <span>IVA (13%)</span>
                        <span id="tax">₡{{ number_format($quote->tax_amount, 0, ',', '.') }}</span>
                    </div>
                    <div class="flex justify-between text-lg font-bold text-slate-900 border-t border-slate-200 pt-2">
                        <span>Total</span>
                        <span id="total">₡{{ number_format($quote->total, 0, ',', '.') }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="flex gap-3">
                <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-8 py-3 rounded-xl transition-colors">Guardar Cambios</button>
                <a href="/admin/quotes/{{ $quote->id }}" class="bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 font-medium px-8 py-3 rounded-xl transition-colors">Cancelar</a>
            </div>
        </form>
    </main>
